...
const regex1 = /CREATE OR REPLACE FUNCTION public\.guardian_verify_access[\s\S]*?END;\n\$\$;\n\nGRANT EXECUTE ON FUNCTION public\.guardian_verify_access\(text, text\) TO anon, authenticated;/g;
content = content.replace(regex1, '');

const regex2 = /CREATE OR REPLACE FUNCTION public\.guardian_fetch_dashboard_data\([\s\S]*?END;\n\$\$;\n\nGRANT EXECUTE ON FUNCTION public\.guardian_fetch_dashboard_data\(text, text\) TO anon, authenticated;/g;
