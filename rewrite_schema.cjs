...
const startStr = "CREATE OR REPLACE FUNCTION public.verify_guardian_access(";
const startIdx = content.indexOf(startStr);
if (startIdx !== -1) {
  const endStr = "GRANT EXECUTE ON FUNCTION public.verify_guardian_access(text, text) TO anon, authenticated;";
...
GRANT EXECUTE ON FUNCTION public.verify_guardian_access(text, text) TO anon, authenticated;`;
  ...
  fs.writeFileSync('supabase/migrations/20260704_verify_guardian_access.sql', `
DROP FUNCTION IF EXISTS public.guardian_verify_access;
DROP FUNCTION IF EXISTS public.guardian_fetch_dashboard_data;
${newFunc}
`);
